...
SKILL_TRIGGERS = ["read screen", "ocr", "text on screen", "what does it say", "read this", "screen text"]
SKILL_DESCRIPTION = "Screenshot the screen and extract text using vision"

import subprocess, tempfile, base64, requests, json
    ...
    try:
        ...
        r = requests.post("http://localhost:8082/v1/chat/completions", json={
            ...
            "messages": [{"role": "user", "content": [
                ...
                {"type": "text", "text": "Extract and return ALL visible text from this screenshot. Return only the text, no descriptions."}
            ]}],
            "max_tokens": 4000
        }, timeout=30)
