...
DRY_RUN = False
_cfg = {}
AGENT_NAME = 'C'  # updated after config load
...
def build_session_script(safe_sys, session_id):
    ...
    L.append("4. MEMORY: Your shared memory database is at ~/.q_memory.db (SQLite). Table: conversations (session_id, timestamp, role, content). Query it with: sqlite3 ~/.q_memory.db")
    L.append("5. When user says check memory, run: sqlite3 ~/.q_memory.db 'SELECT substr(content,1,150) FROM conversations WHERE session_id LIKE "pipecat_%" OR session_id LIKE "voice_%" ORDER BY id DESC LIMIT 15'")
    L.append("6. When user says find password or secret code, run: sqlite3 ~/.q_memory.db 'SELECT substr(content,1,200) FROM conversations WHERE (content LIKE "%%secret%%" OR content LIKE "%%code%%" OR content LIKE "%%password%%") AND session_id LIKE "pipecat_%%" ORDER BY id DESC LIMIT 10'")
    L.append("7. When user says pending tasks, run: sqlite3 ~/.q_memory.db \"SELECT content FROM conversations WHERE content LIKE '%task%' OR content LIKE '%open%' OR content LIKE '%search%'\"")
