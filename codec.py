...
def build_session_script(safe_sys, session_id):
    ...
    L.append("5. When user says check memory or check logs, run: sqlite3 ~/.q_memory.db 'SELECT role, substr(content,1,200) FROM conversations ORDER BY id DESC LIMIT 10'")
    L.append("6. When user says find password or secret code, run: sqlite3 ~/.q_memory.db \"SELECT content FROM conversations WHERE content LIKE '%secret%' OR content LIKE '%code%' LIMIT 10\"")
