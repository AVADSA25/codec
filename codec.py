...
def build_session_script(safe_sys, session_id):
    ...
    L.append("5. When user says check memory or check logs, run: sqlite3 ~/.q_memory.db 'SELECT role, content FROM conversations ORDER BY id DESC LIMIT 20'")
    L.append("6. When user says find password or secret code, run: sqlite3 ~/.q_memory.db \"SELECT content FROM conversations WHERE content LIKE '%code%' OR content LIKE '%password%' OR content LIKE '%secret%'\"")
    ...
    L.append("            def allow():")
    L.append("                result['allow'] = True; root.quit(); root.destroy()")
    L.append("            def deny():")
    L.append("                result['allow'] = False; root.quit(); root.destroy()")
