...
>
> **Dashboard 3-of-5 (PR-3E-dashboard):** migrated the 3 *independent* non-stream dashboard text sites — `_qwen_chat_classify` (clean; gains `<think>` strip before its JSON parse), the `command` Flash fallback (2 error strings → 1, voice-reply-style collapse), and the crew-report writer (`raise_on_error=True` preserves its raise-on-failure). Pinned by `tests/test_dashboard_llm.py`. See `docs/PR3E-DASHBOARD-DESIGN.md`. **Still pending (own PRs):** the chat-handler **stream + non-stream fallback pair** (share one `payload`; the stream needs a `codec_llm.stream(keepalive=)` affordance for the Cloudflare tunnel — the SkillTagBuffer side is already done in PR-3D-c), voice `_stream_qwen` + agents `Agent.run` (async `astream()` + queue), self_improve/watcher, skills tranche (translate/fact_extract/create_skill/skill_forge).
