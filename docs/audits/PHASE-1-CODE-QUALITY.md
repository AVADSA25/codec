...
>
> **Async complete (PR-3E-async):** added `codec_llm.acall()` (async non-stream, mirrors `call()` + `raise_on_error`, injected client) + `codec_llm.astream()` (async stream, mirrors `stream()` + keepalive, but **propagates** exceptions so voice can speak failures). Migrated the 3 queue-coupled async sites — `codec_voice._stream_qwen` (`astream`, queue CRITICAL stays at the call site, per-token `<think>` strip + spoken error kept), `codec_agents.Agent.run` (`acall(raise_on_error=True)`, queue MEDIUM), and the agents research-refiner (`acall`, never-raise → defaults). Queue ownership stays at every call site — `codec_llm` never touches the semaphore. Removed dead `codec_agents._qwen_url()`. Pinned by `tests/test_llm_async.py` (12); crew + voice regression green. See `docs/PR3E-ASYNC-DESIGN.md`. **A-12 streaming complete.** **Still pending A-12:** self_improve/watcher + skills tranche (translate/fact_extract/create_skill/skill_forge).
