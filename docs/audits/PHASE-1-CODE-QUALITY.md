...
> **Closed by PR-3 A-4** (branch `fix/pr3-a4-skill-loader-unification`; design-first per AGENTS.md §11 — see `docs/A4-SKILL-LOADER-UNIFICATION-DESIGN.md`). The legacy eager loader (`codec_core.loaded_skills` / `load_skills` / `run_skill` / `_load_custom_triggers`) was **deleted**; codec.py's `_dispatch_inner` + the dashboard `/api/cortex/skills` endpoint now use the canonical `codec_dispatch` registry — the same lazy, AST-gated path chat/MCP/session/telegram/imessage/agent-runner already used. **This was bigger than dedup:** the legacy `load_skills` `exec_module`d skills with **no PR-1A safety gate**, and `run_skill` **bypassed plugin hooks** — so the voice/wake path now (a) runs through the D-1 AST/manifest gate and (b) fires `run_with_hooks` (lifecycle + veto + audit correlation_id) for the first time. **custom_triggers.json** (Option A, Mickael-approved): `SkillRegistry` now overlays `~/.codec/custom_triggers.json` in `get_triggers`/`match_all_triggers`, so user trigger overrides are honored **everywhere** (voice + chat + MCP), not just the old voice path — a consistency *improvement*, zero regression. Removed codec.py's local `check_skills_ranked`/`check_skill`. 10 tests in `tests/test_skill_loader_unification.py` (legacy-symbols-removed, canonical round-trip, custom-trigger override/default/malformed, cortex endpoint, run_with_hooks invariant). Full suite 1376 passing, zero new failures.
...
> **Closed by PR-3D-c.** The streaming `<think>` + `[SKILL:...]` tag-buffering machine is extracted into new **`codec_chat_stream.py`** as the audit recommended — a `SkillTagBuffer` class (feed tokens → yield clean fragments) + a shared `SKILL_TAG_RE`. `_stream_gen` keeps only the SSE/HTTP plumbing (POST, `iter_lines`, `data:`/`[DONE]` framing, keepalive, blank-bubble fallback) and injects `_resolve_skill_tag` (budget + allowlist + dispatch). `chat_completion` 466 → 379 LOC; the subtle protocol (partial-prefix match, 5000-char cap, cross-chunk tag assembly, `<think>` cross-chunk state, visible-char tracking) is now a tested unit (`tests/test_chat_stream.py`, 13). Behavior preserved exactly (incl. the same-chunk-`</think>`-dropped and think-adjacent-text-uncounted quirks). See `docs/PR3D-MONOLITH-EXTRACT-DESIGN.md`. **Bonus:** `SkillTagBuffer` is the tested unit the deferred A-12 dashboard-stream migration needs — the dashboard stream can now consume `codec_llm.stream()`'s raw tokens through it. **PR-3D complete (A-5 + A-6 + A-7 all done).**
...
>
> **Tranche 2 (PR-3E-2):** built the streaming keystone **`codec_llm.stream()`** (sync generator, yields raw deltas, never-raises) + extracted a shared `_build_request`; migrated `codec_session.qwen_stream`, `codec_compaction`, `codec_dictate`. Read-the-source moved `codec_textassist` + `scripts/regen_skill_descriptions` to **tranche 2c** (they have a raise-on-failure contract — never-raise would paste empty over the user's selection / write empty descriptions). Pinned by `tests/test_llm_stream.py`. See `docs/PR3E2-LLM-STREAM-TRANCHE2-DESIGN.md`.
>
> **Tranche 2c (PR-3E-2c):** added `codec_llm.LLMError` + `codec_llm.call(raise_on_error=True)` (raises on every non-success — non-200/exception/empty-200; default stays never-raise). Migrated the 4 fail-loud sites: `codec_textassist` + `scripts/regen_skill_descriptions` (LLMError propagates — fixes the destructive empty-paste / empty-description), and `codec_agent_plan._qwen_chat` + `codec_agent_runner._qwen_chat` via a thin adapter that maps `LLMError` → their public `QwenUnavailableError` (+ a parallel `_qwen_base()` helper). Pinned by `tests/test_llm_raise_mode.py` (109 agent tests still green). See `docs/PR3E2C-RAISE-MODE-DESIGN.md`.
>
> **Bridges (PR-3E-bridges):** migrated `codec_telegram.call_llm` + `codec_imessage.call_llm` text sites to `codec_llm.call` (default never-raise — bridges WANT graceful degradation; the `None`-on-failure contract is preserved via `content if content else None`, and `chat_template_kwargs` is filtered from `llm_cfg["kwargs"]`). Vision sites in both bridges are A-11 (codec_vision), left untouched. Pinned by `tests/test_llm_bridges.py`. See `docs/PR3E-BRIDGES-DESIGN.md`.
>
> **Dashboard 3-of-5 (PR-3E-dashboard):** migrated the 3 *independent* non-stream dashboard text sites — `_qwen_chat_classify` (clean; gains `<think>` strip before its JSON parse), the `command` Flash fallback (2 error strings → 1, voice-reply-style collapse), and the crew-report writer (`raise_on_error=True` preserves its raise-on-failure). Pinned by `tests/test_dashboard_llm.py`. See `docs/PR3E-DASHBOARD-DESIGN.md`.
>
> **Dashboard complete (PR-3E-chat-stream):** added `codec_llm.KEEPALIVE` + `codec_llm.stream(keepalive=)` (yields the sentinel on empty thinking-chunks so the dashboard can hold the Cloudflare tunnel; default off → `codec_session.qwen_stream` unaffected), then migrated the `chat_completion` **stream** path (`codec_llm.stream(keepalive=True)` feeding the existing `SkillTagBuffer`) **and** the non-stream fallback (`codec_llm.call(raise_on_error=True)` → preserves the 500-on-failure) off shared `_common` args. Removed the dead `import requests as rq` + `headers`. Pinned by `tests/test_llm_stream.py` (keepalive) + `tests/test_dashboard_llm.py`. See `docs/PR3E-CHAT-STREAM-DESIGN.md`. **All dashboard A-12 sites now on `codec_llm`** (only the 5 vision sites remain inline — that's A-11).
>
> **Async complete (PR-3E-async):** added `codec_llm.acall()` (async non-stream, mirrors `call()` + `raise_on_error`, injected client) + `codec_llm.astream()` (async stream, mirrors `stream()` + keepalive, but **propagates** exceptions so voice can speak failures). Migrated the 3 queue-coupled async sites — `codec_voice._stream_qwen` (`astream`, queue CRITICAL stays at the call site, per-token `<think>` strip + spoken error kept), `codec_agents.Agent.run` (`acall(raise_on_error=True)`, queue MEDIUM), and the agents research-refiner (`acall`, never-raise → defaults). Queue ownership stays at every call site — `codec_llm` never touches the semaphore. Removed dead `codec_agents._qwen_url()`. Pinned by `tests/test_llm_async.py` (12); crew + voice regression green. See `docs/PR3E-ASYNC-DESIGN.md`. **A-12 streaming complete.**
>
> **A-12 CLOSED (PR-3E-skills-misc):** migrated the final 6 non-hot graceful sites — `codec_self_improve._draft_skill` (`call(retries=2)` → None), `codec_watcher.handle_draft` (kept the 3× notify-retry loop, swapped the inner POST + `extract_content` → `call`), `skills/translate` + `create_skill` + `skill_forge` (graceful → `call` + fallback), `skills/fact_extract._call_llm` (`call(retries=3, raise_on_error=True)` → keeps the `__ERR__` sentinel). Removed 3 dead imports; regenerated `skills/.manifest.json` (the 4 skill files changed → PR-1A hash gate). Pinned by `tests/test_llm_skills_misc.py`. See `docs/PR3E-SKILLS-MISC-DESIGN.md`. **A-12 is fully closed** — every `chat/completions` *text* call site in the repo now routes through `codec_llm`; only vision POSTs (A-11/`codec_vision`) remain inline. (`codec_llm_proxy` stays the priority queue at each call site.)
...
> **Closed by PR-3 A-15** (branch `fix/pr3-a15-config-versioning`). Added `CONFIG_SCHEMA_VERSION = 1` + an ordered migration ladder (`_CONFIG_MIGRATIONS = [_migrate_v0_to_v1, …]`) + `_migrate_config(cfg) -> (cfg, changed)`. `load_config()` now stamps `config_version` and runs the ladder, writing back **only when the file exists AND a migration changed something** — idempotent (subsequent loads see the current version and skip), atomic (tmp+rename, 0600). Two safety guards beyond the audit's spec: (1) never creates a config file just to stamp a version — fresh installs / CI get the version in-memory only; (2) never overwrites an *unparseable* config — a corrupt file is left intact for the user to repair. v0→v1 just stamps the version (all Phase 1-3 keys were additive with safe `.get(k, default)` fallbacks, so no field rewrites needed); future schema changes append a `_migrate_vN_to_vN+1` step. The ladder also force-progresses monotonically so a migration that forgets to bump the version can't infinite-loop. 12 tests in `tests/test_config_version.py` (stamp, idempotent no-rewrite, missing-file-no-create, corrupt-file-not-overwritten, 0600 perms, key preservation, garbage-version handling).
