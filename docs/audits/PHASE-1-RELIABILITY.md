...
> **DEFERRED → PR-4G-2** (split out of PR-4G, which closed H-4/H-6/M-6). `codec_mcp_http` imports `mcp`/`fastmcp`, which are absent both locally and in CI, so its fix can't be unit-tested — and shipping an untested change to the live claude.ai-facing service violates the verify-everything bar. PR-4G-2 will either add an `mcp`-stub test path or land the (trivial: evict empty deques after each `_rate_check`) fix with careful manual verification.
