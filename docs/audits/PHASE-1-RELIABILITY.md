...
> **Closed by PR-4B** (audit's preferred fix — move the bridge to the `conversations` DB, delete the file). `/api/command` now returns `request_id` = the user row's `conversations.id` (server-authoritative correlation token); `/api/response` resolves via the new pure helper `_latest_response_for_session(db, session_id, after_id, after_ts)` → `WHERE session_id=? AND role='assistant' AND id>after_id ORDER BY id ASC LIMIT 1`. The racy `~/.codec/pwa_response.json` (non-atomic write, no writer mutex, no correlation, racy mtime/unlink) is **gone**, and so is the *latent* clock/RTT-skew miss of the old `timestamp > after` query (a fast skill answer on a high-latency Cloudflare link, or client/server clock skew, could make `timestamp > after` miss and strand the PWA on its 5-min poll timeout — the file fast-path had been masking it). The error path now persists an assistant error row (so failures still surface). Frontend (`codec_dashboard.html`) sends `&after_id=` (primary) + keeps `&after=` (legacy fallback for an un-refreshed tab). Defects 1-4 closed; the rare two-tab *simultaneous-interleave* of a shared `flash-<date>` session would need a reply→request id column for strict per-tab attribution (deferred — a `conversations` schema change), but DB+ASC is already strictly better than the clobber-prone single file. Pinned by `tests/test_pwa_response_bridge.py` (18). See `docs/PR4B-PWA-RESPONSE-BRIDGE-DESIGN.md`.
...
> **Closed by PR-4F.** New `codec._state_lock` + three atomic helpers extracted from the compound check-then-set sites: `_try_begin_recording()` (F18 — claims the recording slot atomically so a double-F18 / wake-race can't start two sox processes into the same `audio_path`), `_activate_if_off()` (wake-word activation), `_toggle_active()` (F13 toggle). The helpers do ONLY the atomic decision under the lock; sox/overlays/sounds/dispatch stay outside it. Behavior-preserving (same guards + side effects, same order — only the check-then-set is now atomic). Single-field GIL-atomic reads in guard/display code stay unlocked. Testable via a contained `pynput` stub (`monkeypatch.setitem` + pop `codec` in teardown) so the exactly-one-winner concurrency tests don't leak the stub to `test_graceful_shutdown`/`test_dispatch_inner_helpers`/`test_smoke` (verified zero-new AND zero-fixed). 6 tests (`tests/test_codec_state_lock.py`). See `docs/PR4F-STATE-LOCK-DESIGN.md`.
...
> **Closed by PR-4G.** New `routes/_shared._evict_stale_agent_jobs(now, ttl=86400)` drops terminal (non-`running`) crew jobs older than 24h, snapshot-iterating under the new `_agent_jobs_lock`; a `running` job is never evicted and an entry with an unparseable `started` is kept (no data loss on a bad field). The `POST /api/agents/run` endpoint calls it before each new job and adds the new key under `_agent_jobs_lock` — so the dict is bounded AND the structural add/del can't race the sweep into a `dictionary changed size during iteration`. (Worker-thread value mutations — `progress.append`, `["status"]=…` — don't change dict structure, so they stay lock-free.) 4 tests (`tests/test_bounded_dicts.py`). See `docs/PR4G-BOUNDED-DICTS-DESIGN.md`.
...
> **Closed by PR-4G-2.** `_rate_check` now periodically (gated to once/`_RATE_EVICT_INTERVAL=60`s via `_RATE_LAST_EVICT`) evicts IPs idle for >60s from `_RATE_WINDOW` — evicting on `not dq or dq[-1] < cutoff` (newest entry stale, not just empty, since a silent IP's deque never drains to empty) and skipping the current ip. Bounds the dict as claude.ai rotates source IPs; all under the existing `_RATE_LOCK`, rate-limit semantics unchanged. **Testability** (the reason this was split from PR-4G): `codec_mcp_http` imports `mcp`/`fastmcp`/`codec_mcp` (absent locally + in CI) — `tests/test_rate_window.py` installs a *contained* `sys.modules` stub (real subclassable classes via `monkeypatch.setitem`, imported modules popped in teardown) so the real `_rate_check` is tested without leaking stubs to `test_mcp`/`test_oauth_provider` (verified zero-new **and** zero-fixed). 5 tests. See `docs/PR4G2-RATE-WINDOW-DESIGN.md`.
...
> **Closed by PR-4G.** New `routes/_shared._evict_expired_approvals(now, ttl=120)` deletes entries older than 120s regardless of status (replacing the old mark-`expired`-but-never-delete). Both `GET /api/approvals` and `GET /api/approvals/count` call it inside their existing `with _approval_lock:`, so whichever endpoint the PWA polls keeps the dict bounded. Minor behavior delta (per the audit's recommendation): a click on an approval that expired >120s ago now returns 404 instead of 409 — a 2-minute-stale command approval is re-issued either way. 2 tests (`tests/test_bounded_dicts.py`). See `docs/PR4G-BOUNDED-DICTS-DESIGN.md`.
...
> **Closed by PR-4G.** `_resume_timestamps` is now a class attribute (was lazily created); the prune logic is extracted to `@classmethod VoicePipeline._prune_resumable(now)` and called in **`__init__`** (every new/resumed session) in addition to `_save_for_resume` — so a dropped-and-never-reconnected session is evicted on the next bit of voice activity, not only on the next save. The resume path also pops `_resume_timestamps` so the two dicts can't drift. **Deviation from the audit's `threading.Timer`:** a background timer's lifecycle isn't worth it for a leak the audit itself calls "minor… usage-bounded" — init+save pruning covers every practical case (the only un-evicted window is total voice inactivity, which has no memory pressure). 2 tests (`tests/test_bounded_dicts.py`). See `docs/PR4G-BOUNDED-DICTS-DESIGN.md`.
...
- `~/codec-repo/.claude/worktrees/zealous-villani-2a4867/codec.py`
- `~/codec-repo/.claude/worktrees/zealous-villani-2a4867/codec_audit.py`
- `~/codec-repo/.claude/worktrees/zealous-villani-2a4867/codec_ask_user.py`
...
- `~/codec-repo/.claude/worktrees/zealous-villani-2a4867/codec_agents.py`
- `~/codec-repo/.claude/worktrees/zealous-villani-2a4867/codec_dashboard.py`
- `~/codec-repo/.claude/worktrees/zealous-villani-2a4867/codec_dictate.py`
- `~/codec-repo/.claude/worktrees/zealous-villani-2a4867/codec_dispatch.py`
- `~/codec-repo/.claude/worktrees/zealous-villani-2a4867/codec_voice.py`
- `~/codec-repo/.claude/worktrees/zealous-villani-2a4867/codec_observer.py`
- `~/codec-repo/.claude/worktrees/zealous-villani-2a4867/codec_keyboard.py`
- `~/codec-repo/.claude/worktrees/zealous-villani-2a4867/codec_autopilot.py`
- `~/codec-repo/.claude/worktrees/zealous-villani-2a4867/codec_telegram.py`
- `~/codec-repo/.claude/worktrees/zealous-villani-2a4867/codec_session.py`
- `~/codec-repo/.claude/worktrees/zealous-villani-2a4867/codec_memory.py`
- `~/codec-repo/.claude/worktrees/zealous-villani-2a4867/codec_llm_proxy.py`
- `~/codec-repo/.claude/worktrees/zealous-villani-2a4867/codec_mcp_http.py`
- `~/codec-repo/.claude/worktrees/zealous-villani-2a4867/codec_triggers.py`
- `~/codec-repo/.claude/worktrees/zealous-villani-2a4867/routes/_shared.py`
- `~/codec-repo/.claude/worktrees/zealous-villani-2a4867/routes/agents.py`
- `~/codec-repo/.claude/worktrees/zealous-villani-2a4867/routes/websocket.py`
