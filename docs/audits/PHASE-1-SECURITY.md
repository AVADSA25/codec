...
> **Closed by PR-2D** (branch `fix/pr2d-internal-token-replacement`). Per-process HMAC token replaces the trusted `X-Internal: codec` literal. `codec_keychain.get_internal_token()` bootstraps a 32-byte `secrets.token_urlsafe` value to macOS Keychain on first miss (0600 envelope-encrypted fallback headless) and caches it in-process 30s — no `~/.codec/internal_token` plaintext file is created. `AuthMiddleware.dispatch` (`codec_dashboard.py:138-170`) reads the presented `X-Internal-Token` header and validates it constant-time via `hmac.compare_digest`; mismatches emit `internal_token_mismatch` audit and fall through to normal auth. The legacy `X-Internal: codec` header is no longer recognized — a caller sending it alone (no token) gets the same 401 as any unauthenticated request. All 5 internal callers updated: `codec_heartbeat.py`, `codec_scheduler.py` (2 sites), `skills/notification_reader.py`, `skills/health_check.py`, plus `scripts/feature_audit.py` (audit script). 14 tests in `tests/test_auth_middleware.py` cover bootstrap, cache, mismatch audit, the legacy-header-ignored invariant, the non-localhost defense-in-depth, and source-level absence of the old literal.
...
> **Partially closed by PR-2B** ([#50](https://github.com/AVADSA25/codec/pull/50), merged as `911cc76`). Two of the seven plaintext secrets — `dashboard_token` and `llm_api_key` — now live in macOS Keychain. `codec_config.get_dashboard_token()` and `codec_config.get_llm_api_key()` migrate the cfg plaintext on first call (idempotent, blanks the on-disk field) and serve subsequent reads from Keychain with a 30s in-memory cache. `AuthMiddleware.dispatch` + 8 live LLM-call sites updated. **Deferred to PR-2B-2**: `gemini_api_key`, `pexels_api_key`, `serper_api_key`, `telegram.bot_token`. The unsalted `auth_pin_hash` argon2id refactor is also deferred per Mickael's Out-of-scope decision.
>
> **Remainder closed by PR-2B-2** (branch `fix/pr2b2-remaining-secrets-keychain`). The four deferred secrets now migrate cfg-plaintext → Keychain via new getters reusing the PR-2B `_migrate_and_get` / 30s-cache machinery: `codec_config.get_gemini_api_key()`, `get_pexels_api_key()`, `get_serper_api_key()` (top-level keys), and `get_telegram_bot_token()` (the nested `telegram.bot_token`, via a new `_migrate_and_get_nested` + `_blank_nested_config_field` that preserves sibling keys like `chat_id`). Keychain keys: `ai.avadigital.codec.{gemini_api_key, pexels_api_key, serper_api_key, telegram_bot_token}`. Each getter precedence is Keychain → cfg (migrate-on-first-call) → env-var fallback (`GEMINI_API_KEY` / `PEXELS_API_KEY` / `SERPER_API_KEY` / `TELEGRAM_BOT_TOKEN`); env values are NOT migrated to Keychain (ephemeral per process). 6 call sites updated: `codec.py`, `codec_voice.py` (gemini), `codec_gdocs.py` (pexels), `codec_search.py` + `codec_agents.py` (serper), `codec_telegram.py` (telegram). 20 tests in `tests/test_remaining_secrets.py`. **Two documented residuals (not in D-15's named scope):** (1) `codec_alerts.py` reads a SEPARATE nested key `alerts.telegram.bot_token` — a distinct config location the audit did not name; left as a future cleanup. (2) `auth_pin_hash` argon2id refactor remains deferred per the original Out-of-scope decision. With these, all four D-15-named plaintext secrets + the original two are now Keychain-backed.
...
> **Closed by PR-2H** (branch `fix/pr2h-ast-reflection-and-fileops`). `is_dangerous_skill_code` now walks `ast.Attribute` and `ast.Name` nodes for reflection sandbox-escapes: (1) `_DANGEROUS_REFLECTION_ATTRS` blocks any attribute access named `__class__`, `__bases__`, `__base__`, `__subclasses__`, `__mro__`, `__globals__`, `__dict__`, `__builtins__`, `__code__`, `__closure__`, `__func__`, `__self__`, `__getattribute__`, `__subclasshook__`, `__reduce__`/`__reduce_ex__`, `__loader__`, `__spec__` — regardless of the base object, so `().__class__.__bases__[0].__subclasses__()`, `f().__globals__['os']`, `__builtins__.__dict__['eval']`, and f-string `__mro__` chains are all caught. (2) bare `__builtins__` / `__loader__` / `__spec__` Name references → refused. (3) `vars` + `dir` (and `setattr`/`delattr`) added to `DANGEROUS_CALLS` (the python_exec placeholder test `test_python_exec_blocks_vars_builtins` is now un-skipped + asserting). **Two documented scope decisions:** (a) `open` is intentionally NOT blocked — legitimate file skills need it, and runtime file access is constrained by python_exec's sandbox-exec profile + file_ops/file_write path blocklists. (b) Network modules (`urllib.request`, `requests`, `httpx`, `http.client`, ...) are intentionally NOT blocked — blocking only `urllib.request` while allowing `requests` would be security theater, blocking ALL network breaks the existing contract that skills make HTTP calls (weather, web_search, self_improve-drafted API skills), and network for untrusted python_exec is already blocked at runtime by PR-2C's sandbox-exec. Comprehensive network gating belongs in the audit's "Eventually: positive allowlist of permitted modules" rewrite, not this reflection-focused PR. `socket` stays blocked (it was pre-PR-2H). 36 tests in `tests/test_skill_code_reflection.py` (incl. a test pinning the deliberate network-allowed decision) + the un-skipped python_exec placeholder.
...
> **Closed by PR-2H** (branch `fix/pr2h-ast-reflection-and-fileops`). `file_ops._is_safe_path` rewritten to mirror PR-1C's `file_write` blocking: `_build_blocked_roots()` realpath-resolves the macOS system tree + **the entire `~/.codec/` tree** + the repo's built-in `skills/` dir at module load. The candidate path's parent is realpath-resolved (so not-yet-existing write targets + symlink-into-blocked-root traversal are both caught), then compared with `real == bp or real.startswith(bp + os.sep)`. Refusals emit a `file_ops_blocked` audit event (`source=codec-skill-file-ops`, `outcome=error`, `level=warning`, `extra={target_path, requested_path, reason}`). Pre-existing realpath bug also fixed: `/private` was blanket-blocked, which broke legit `/tmp` writes (macOS realpaths `/tmp → /private/tmp`) — `/private` removed from the system roots, `/tmp` now allowed. 27 tests in `tests/test_file_ops.py` (codec-tree blocked, repo-skills blocked, system/sensitive still blocked, legit paths incl. `/tmp` allowed, symlink-into-codec blocked, audit emit, no-real-dir-write guard). Defense-in-depth with PR-1A (load-time AST gate) + PR-1C (file_write) + PR-1D (permission_gate) — four independent layers on the D-1 write-to-skills-dir → RCE chain.
