...
**Date:** 2026-05-17
**Status:** **6 of 6 audits complete + the Pilot half** (A, C, D, E, F, **B incl. Pilot**). Audit B (Projects subsystem) landed 2026-05-24 — `docs/audits/PHASE-1-PROJECTS-PILOT.md` (20 findings; all fixed in code via PR-7A…PR-7P). The deferred **Pilot** half was audited 2026-05-24 once the `~/codec/pilot/` checkout became available — `docs/audits/PHASE-1-PILOT-AUDIT.md` (15 findings; **a LIVE internet-exposed unauthenticated RCE was found + stopped**). Pilot fixes ship in the separate pilot repo (not codec-repo); `pilot-runner` is stopped pending the fix wave.
...
- PR-4F: H-2 ✅ (branch `fix/pr4f-state-lock`, design-first → `docs/PR4F-STATE-LOCK-DESIGN.md`). New `codec._state_lock` + three atomic helpers extracted from the compound check-then-set sites: `_try_begin_recording()` (F18 — the bug: a double-F18/wake race could start two sox into the same audio_path), `_activate_if_off()` (wake activation), `_toggle_active()` (F13). Behavior-preserving (same guards/side-effects/order; only the check-then-set is atomic; expensive work stays outside the lock; GIL-atomic single reads stay unlocked). Built a **contained `pynput` stub** harness (codec.py imports `from pynput import keyboard`, absent locally + CI) — `monkeypatch.setitem` + pop `codec` in teardown — so the exactly-one-winner concurrency tests don't leak to `test_graceful_shutdown`/`test_dispatch_inner_helpers`/`test_smoke`. 6 tests (`tests/test_codec_state_lock.py`); zero net-new ruff (codec.py 72=72 via `--statistics`); full suite zero new **and zero fixed** (41 = 41 baseline — confirms stub containment). **Last Wave-4 HIGH closed.**
...
- PR-4G-2: H-5 ✅ (branch `fix/pr4g2-rate-window`, design-first → `docs/PR4G2-RATE-WINDOW-DESIGN.md`). `_rate_check` evicts IPs idle >60s from `_RATE_WINDOW` (on `not dq or dq[-1] < cutoff` — newest-stale, not just empty, since silent IPs never drain to empty; skips current ip), gated to once/60s via `_RATE_LAST_EVICT`. Built a **contained `sys.modules` stub harness** (real subclassable classes, `monkeypatch.setitem` auto-revert + pop imported modules in teardown) so `codec_mcp_http` (imports absent `mcp`/`fastmcp`/`codec_mcp`) is importable for tests WITHOUT leaking stubs to `test_mcp`/`test_oauth_provider`. 5 tests (`tests/test_rate_window.py`); zero net-new ruff; full suite zero new **and zero fixed** (41 = 41 baseline — confirms no stub leakage). Worktree-to-`sys.path[0]` guard added (codec_autopilot lesson).
- PR-4H: H-7 + H-8 + H-9 ✅ (branch `fix/pr4h-tempfile-leaks`, design-first → `docs/PR4H-TEMPFILE-LEAKS-DESIGN.md`). try/finally cleanup so the unlink runs on the failure path too: H-7 (`codec_observer._screencapture_and_ocr_blocking` — `finally` + `tmp_png=None` guard; PR-4A-2 prefix/glob-purge stays as backstop), H-8 (verify-first: the source tmp was already cleaned; only the **Rust `<tmp>.out`** leaked → finally now unlinks both), H-9 (`codec_session.speak()` — afplay-wait-then-unlink daemon thread; +`import threading`). **Bonus (same H-7 class, same file):** `codec_session.screenshot_ctx()` had the identical success-path-only unlink — also moved to `finally`. 4 tests (`tests/test_tempfile_leaks.py`); 63 observer/dashboard/session regression tests green; zero net-new ruff (all 3 source files SAME); full suite zero new (41 = 41 baseline).
- PR-4I: M-3 + M-4 + L-1 + L-2 ✅ (branch `fix/pr4i-small-reliability`, design-first → `docs/PR4I-SMALL-RELIABILITY-DESIGN.md`). M-3 (`codec_audit._rotate_if_needed` now `log.warning`s rename failures via a new stdlib module logger — never `audit()`, avoids `_LOCK` re-entry), M-4 (`codec_observer.run_daemon` whole iteration body inside one try + cadence default, so `_idle_seconds`/`_load_config` can't kill the daemon), L-1 (`codec_ask_user._atomic_write_text` helper: open+write+flush+fsync+replace, 3 sites, keeps `default=str`), L-2 (`codec_autopilot._load_state` → `{"__corrupt__": True}` + ERROR on JSONDecodeError; `_tick` refuses to fire on corrupt state instead of re-firing). 8 tests (`tests/test_small_reliability.py`); 88 audit/observer/ask_user/autopilot regression tests green; zero net-new ruff (all 4 source files SAME); full suite zero new (41 = 41 baseline). **Test-infra note:** the autopilot tests force the worktree to `sys.path[0]` + import at collection — a module no earlier test imports was resolving to the parent checkout (worktree nested under main repo); CI's standalone checkout is unaffected. **Wave 4 testable remainder complete.**
- PR-4J: M-5 ✅ (branch `fix/pr4j-perthread-db`, design-first → `docs/PR4J-PERTHREAD-DB-DESIGN.md`). `routes/_shared.get_db()` → per-thread connection via `threading.local()` (each sets WAL + busy_timeout=5000 + Row) + `_db_conns` registry + `_close_all_db_conns()`; dashboard shutdown calls it instead of touching the old `_db_conn` global. Concurrent readers parallelize (WAL N-readers/1-writer) + per-connection transaction isolation (fixes a latent cross-thread visibility hazard); no caller change, single-thread behavior identical. 6 tests (`tests/test_perthread_db.py`); 128 dashboard/agents/memory regression tests green; zero net-new ruff (both files SAME via `--statistics`); full suite zero new (41 = 41 baseline). **🎉 This closes the LAST finding in Audit C — Wave 4 (Reliability) is fully complete: all 5 CRITICAL + all 9 HIGH + all 6 MEDIUM + both LOW.**
...
>
> **Wave 7 burn-down COMPLETE (2026-05-24):** all 20 Audit-B findings B-1…B-20 fixed in code across **PR-7A…PR-7O** (merged sequence in `PHASE-1-PROJECTS-PILOT.md` §"Actually landed"). The autonomous-agent permission boundary — once bypassable (LLM-self-declared flags) with a phantom strict-consent import — is now: real consent gate (B-1), server-derived destructiveness (B-2), path/grant tamper + realpath/glob confinement (B-3/B-4/B-15/B-18), atomic approval + crash-safe resume with at-most-once destructive replay (B-9/B-5), flock'd state + 0600 perms (B-7/B-10/B-11), schema-migration ladder (B-13), LLM-call/extend-budget caps (B-14), and outbound-content opt-in (B-17). Plus **PR-7P** resolved the two previously-deferred sub-parts (Mickael's decisions 2026-05-24): B-2 closed via a central server-side `SKILL_CAPABILITIES` table (the gate no longer trusts the LLM's path/network flags), B-3 closed via "defer ownership authz + forensic caller-IP audit" (single-user threat model). **The Pilot audit is now also DONE** (2026-05-24, `docs/audits/PHASE-1-PILOT-AUDIT.md`, 15 findings) — it found a **live internet-exposed unauthenticated RCE** (`pilot.lucyvpa.com → :8094`, no auth, drive-the-browser + compile/approve-skill → code-exec), now **stopped** (`pm2 stop pilot-runner`). Pilot fixes ship in the separate `~/codec/pilot/` repo (fix wave PP-1…PP-5 in the audit doc); do NOT restart `pilot-runner` until PP-1 (loopback+auth) lands. Non-blocking documented residuals (only if multi-user / for exact-arg parsing): B-2 free-text-task path extraction, B-3 full ownership authz, B-14 extend_budget authz, B-16 full `/api/crews` vs `/api/projects` URL-namespacing.
