...
>
> **Wave 7 burn-down COMPLETE (2026-05-24):** all 20 Audit-B findings B-1…B-20 fixed in code across **PR-7A…PR-7O** (merged sequence in `PHASE-1-PROJECTS-PILOT.md` §"Actually landed"). The autonomous-agent permission boundary — once bypassable (LLM-self-declared flags) with a phantom strict-consent import — is now: real consent gate (B-1), server-derived destructiveness (B-2), path/grant tamper + realpath/glob confinement (B-3/B-4/B-15/B-18), atomic approval + crash-safe resume with at-most-once destructive replay (B-9/B-5), flock'd state + 0600 perms (B-7/B-10/B-11), schema-migration ladder (B-13), LLM-call/extend-budget caps (B-14), and outbound-content opt-in (B-17). **Only the Pilot audit remains** (sister `~/codec/pilot/` checkout, not in this repo). Deferred sub-parts pending Mickael's decisions: B-2 path/network *capability model*, B-3 *per-agent ownership authz* (gates B-14 extend_budget authz + B-16 full URL-namespacing).
