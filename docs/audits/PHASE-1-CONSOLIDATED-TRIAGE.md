...
>
> **Wave 7 burn-down COMPLETE (2026-05-24):** all 20 Audit-B findings B-1…B-20 fixed in code across **PR-7A…PR-7O** (merged sequence in `PHASE-1-PROJECTS-PILOT.md` §"Actually landed"). The autonomous-agent permission boundary — once bypassable (LLM-self-declared flags) with a phantom strict-consent import — is now: real consent gate (B-1), server-derived destructiveness (B-2), path/grant tamper + realpath/glob confinement (B-3/B-4/B-15/B-18), atomic approval + crash-safe resume with at-most-once destructive replay (B-9/B-5), flock'd state + 0600 perms (B-7/B-10/B-11), schema-migration ladder (B-13), LLM-call/extend-budget caps (B-14), and outbound-content opt-in (B-17). Plus **PR-7P** resolved the two previously-deferred sub-parts (Mickael's decisions 2026-05-24): B-2 closed via a central server-side `SKILL_CAPABILITIES` table (the gate no longer trusts the LLM's path/network flags), B-3 closed via "defer ownership authz + forensic caller-IP audit" (single-user threat model). **Only the Pilot audit remains** (sister `~/codec/pilot/` checkout, not in this repo). Non-blocking documented residuals (only relevant if multi-user / for exact-arg parsing): B-2 free-text-task path extraction, B-3 full ownership authz, B-14 extend_budget authz, B-16 full `/api/crews` vs `/api/projects` URL-namespacing.
