...
>
> **✅ REMAINDER ADDRESSED by PR-7P (2026-05-24).** Mickael chose the **central capability table** (over per-skill metadata). `codec_agent_runner.SKILL_CAPABILITIES` maps each skill → `{writes_path, reads_path, network}`; `permission_gate` OR-upgrades the LLM's `touches_path`/`reads_path`/`network_call` flags with the server-known caps (same "LLM can only raise risk" pattern as `_effective_destructive`), gating a **declared** path/domain even when the flag is False — so a write/network-capable skill can't skip its gate by under-declaring. Classified by exfil surface (FS skills → path caps; web/chrome/google/imessage/translate/do-anything → network); benign read-only public-data skills (`weather`, `bitcoin_price`) deliberately not network-gated (no user-data exfil surface; also keeps them grant-free). 5 tests (`tests/test_capability_gate.py`); 126 runner suites green. **Documented residual:** extracting the *exact* path/URL a skill acts on from its free-text task (vs the declared value) still needs structured skill invocation — mitigated by manifest grants + realpath/glob confinement (PR-7L) + the `file_write` blocklist (PR-1C). Design: `docs/PR7P-CAPABILITY-AUTHZ-DESIGN.md`.
...
>
> **✅ RESOLVED by PR-7P (2026-05-24).** Mickael chose **defer ownership authz + forensic audit**. Per-agent ownership authz stays out of scope for the single-user threat model (loopback + global `AuthMiddleware` + the PR-7C grant-value blocklist are the boundary); revisit if/when the dashboard goes multi-user (a code comment documents this). The state-changing mutations `approve` / `abort` / `grant` now emit an `agent_mutation` forensic audit event with the caller IP (`request.client.host`) via `_audit_mutation`, so a localhost-foothold abuse is detectable after the fact. 1 test (`tests/test_mutation_audit.py`). Design: `docs/PR7P-CAPABILITY-AUTHZ-DESIGN.md`.
...
> **✅ FIXED by PR-7N (2026-05-24).** `_qwen_next_action` is now a thin orchestrator over module-level pure units: `_build_action_prompt` (composition), `_trim_history`, `_extract_file_list` / `_already_read` / `_build_file_iteration_hint` (the string-derived iteration tracker, isolated + tested), `_parse_action_json` (fences/bare/balanced-brace), and `_action_from_json` (Action construction, unknown keys ignored). Behavior-preserving (same prompt bytes, same parse, same Action — all existing runner tests stay green). 8 unit tests (`tests/test_runner_loop.py`). The deeper "typed iteration tracker consuming structured history instead of regex-on-output-strings" is documented inline as a follow-up — the extraction makes it a localized, swappable change now. Design: `docs/PR7N-RUNNER-LOOP-DESIGN.md`.
...
> **✅ FIXED by PR-7N (2026-05-24).** `_execute_checkpoint` now counts **every** `_qwen_next_action` call (including the correction-nudge retry) against `budget` via a `_next_action()` wrapper — `budget` bounds LLM **calls**, not loop iterations, so a correction-heavy runaway can't burn ~2× the intended calls (raises `StepBudgetExhausted` at the cap). `extend_budget` (routes) caps the **cumulative** override at the new `MAX_CHECKPOINT_STEP_BUDGET = 500` ceiling and returns 409 once reached — repeated extends can no longer push the budget without limit. The **authz gate on `extend_budget` is deferred** (depends on B-3 per-agent ownership authz). 2 tests (`tests/test_runner_loop.py`). Design: `docs/PR7N-RUNNER-LOOP-DESIGN.md`.
...
> **✅ FIXED (safety half) by PR-7O (2026-05-24).** `save_custom_agent` now refuses (409) to write a custom-agent id that shadows an existing Phase-3 Project (`agents/<id>/manifest.json`) via `_custom_id_shadows_project` — closing the documented collision in the shared `~/.codec/agents/` namespace. The **full URL namespacing** (`/api/crews/*` vs `/api/projects/*`) is a breaking, PWA-facing rename intentionally **deferred** as a larger Phase-3.5 refactor (noted inline) — this PR closes the actual safety gap without breaking the PWA. 2 tests (`tests/test_edges.py`). Design: `docs/PR7O-EDGES-DESIGN.md`.
...
> **✅ FIXED by PR-7O (2026-05-24).** Off-device channels (`telegram`/`imessage`, in `_REMOTE_CHANNELS`) now only carry agent CONTENT when the agent has explicitly opted in (`manifest.allow_outbound_content`, default **False** — local-first). Without opt-in, `_dispatch_to_channel` replaces the title/body with a content-free ping (`"You have a new agent update. Open the CODEC dashboard…"`) — no file contents / fetched data leave the machine. The Telegram token now prefers the Keychain-backed `codec_config.get_telegram_bot_token()` (PR-2B-2), falling back to the plaintext `notifications.telegram_token`; `chat_id` stays in config (not a secret). macOS banners (local) are unaffected. 3 tests (`tests/test_edges.py`). Design: `docs/PR7O-EDGES-DESIGN.md`.
...
> PR-7A = B-1 · PR-7B + PR-7G = B-2 (server-side derivation, then loop enforcement) · PR-7C = B-3 · PR-7E = B-4 · PR-7D = B-7 (flock CAS only — it did **not** also fix B-9 as originally suggested) · PR-7F = B-6 (user-reply pickup) · PR-7G = B-8 · **PR-7H = B-9** (atomic approval write + pre-approval abort) · **PR-7I = B-5** (resume keeps in-checkpoint history + at-most-once destructive replay). **EVERY Audit-B CRITICAL/HIGH is now closed: B-1, B-2, B-3, B-4, B-5, B-6, B-7, B-8, B-9.** MEDIUM/LOW burn-down: **PR-7J = B-10 + B-11** (0600 perms + notifications flock) · **PR-7K = B-13 + B-19** (plan schema migration ladder + dataclass-splat guard) · **PR-7L = B-15 + B-18** (open-folder realpath confinement + glob-grant precision) · **PR-7M = B-20** (reply dedup by consumed-offset) · **PR-7N = B-12 + B-14** (runner action-loop: testable decomposition + budget backstop) · **PR-7O = B-16 + B-17** (crew/Project collision guard + outbound-content opt-in & Keychain token) · **PR-7P = B-2 remainder + B-3 remainder** (central capability table + forensic mutation audit; per the decisions Mickael made 2026-05-24). **ALL 20 Audit-B findings (B-1…B-20) are now fully addressed in code, including the two previously-deferred sub-parts.** Only the **Pilot audit** remains — it needs the `~/codec/pilot/` checkout (not in this repo). Remaining acknowledged residuals (documented, not blocking): B-2 exact-path/URL extraction from free-text tasks (needs structured skill invocation); B-3 full per-agent ownership authz + B-14 `extend_budget` authz + B-16 full `/api/crews` vs `/api/projects` URL-namespacing (all only matter if the dashboard goes multi-user).
