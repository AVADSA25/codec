...
> **🟡 MECHANISM CLOSED — W5-5/PR-5G.** `packaging/macos/fetch_models.py` + `models.json` implement option (a)+(c): a **tiered** manifest (`bundled` = ship/first-run ≈6 GB Whisper+Kokoro+7B; `on_demand` = 35B LLM + vision, downloaded with consent) that installs to **`~/.codec/models/`** (not the `.app` — the `DiskSpace`/`FileTimestamp` privacy reasons were declared in W5-1). Downloads go through `huggingface_hub.snapshot_download` (**resumable + commit/etag-verified**); a `--yes` consent gate prints total size first; `--dry-run` previews. Pure core unit-tested (5 tests, `tests/test_model_fetch.py`); validated on macOS. **Open (your decision, `docs/HANDOFF-MICKAEL.md §2`):** confirm the exact model list + **pin every `revision` to a commit SHA** (the manifest currently uses real repos at `main` as a recommended default). First-run auto-fetch of the bundled set is wired in W5-6.
...
> **🟡 HEADLESS LAYER CLOSED — W5-6/PR-5I.** `packaging/macos/first_run.py` is the first-launch orchestrator the wizard will drive: installs the launchd fleet → fetches the bundled model set → reports each TCC permission's status (`granted`/`denied`/`unknown`, best-effort via `ctypes` `AXIsProcessTrusted` / `CGPreflightScreenCaptureAccess` / a Full-Disk probe) with a **per-permission "why" + a `x-apple.systempreferences:` deep link** to the exact pane, and the feature that degrades if denied. Idempotent (`~/.codec/.first_run_complete`; `--force`, `--permissions-only`, `--dry-run`). Validated on macOS (correctly read live TCC status). 6 tests (`tests/test_first_run.py`). **Remaining:** the native Swift wizard window that opens these deep links + the "ask again" affordance = **W5-11** (it calls this core).
