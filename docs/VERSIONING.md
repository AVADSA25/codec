...
**Resolution: deleted as erroneous** — the tag, the GitHub release, and its stale "Latest"
badge are gone; `v2.3.0` is now the Latest release. The next real major becomes `v3.0.0` when
it's actually cut (bump `VERSION` + add a CHANGELOG entry + tag per the steps above).
