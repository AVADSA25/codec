...
def test_all_crews_define_allowed_tools():
    ...
    C6 / SR-41: 12 crew builders moved to codec_crews.py. Scan there
    instead of codec_agents.py (which now imports + re-exports them)."""
    REPO = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
    content = open(os.path.join(REPO, "codec_crews.py")).read()
    import re
    # Match only the crew builder functions (end with _crew, take **kwargs)
    crew_defs = re.findall(r'def (\w+_crew)\(\*\*kwargs\)', content)
    # Floor: ≥12 crews — catches future removals as regressions instead
